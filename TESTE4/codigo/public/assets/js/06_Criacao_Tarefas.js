...
async function removeTaskFromServer(task) {
    try {
        ...
        data.listaDeTarefas = data.listaDeTarefas.filter(t => t.id !== task.id);
        data.adicionarTarefas = data.adicionarTarefas.filter(t => t.id !== task.id);
...
async function loadTasks(dayOfWeek) {
    try {
        const response = await fetch('/TESTE4/codigo/db/db.json');
        ...
        if (Array.isArray(data.tarefas.tasks_calendar)) {
            data.tarefas.tasks_calendar.forEach(task => {
